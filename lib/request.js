...
function requestWrapper(requestUrl, config) {
    ...
    _request(requestUrl, function (error, response, body) {
        if (error) {
            deferred.reject(createError('Request to ' + requestUrl + ' 111 failed: ' + error.message, error.code));
        } else {
            if (response.statusCode === 200) {
                deferred.resolve(body);
            } else {
                deferred.reject(createError('Request to ' + requestUrl + ' 222 returned ' + response.statusCode + ' status code.', 'EREQUEST', {
